...
### Baseline 评测脚本
用于对“无 Planner”访谈对话做快速量化评估（适合和后续 Planner 版本对比）：

```bash
python scripts/evaluate_baseline.py --input data/sample_transcript.txt
```

支持输入格式：
- 文本格式：每行以 `老人:` / `助手:` 开头
- JSON 格式：`[{ "role": "user|assistant", "content": "..." }]`，也支持 `messages/conversation/transcript` 字段

导出 JSON 结果：
```bash
python scripts/evaluate_baseline.py --input your_transcript.json --output-json data/eval_result.json
```

当前指标：
- 单问题率（助手回复是否为单个问句）
- 追问深度（连续围绕用户上一轮内容跟进的平均深度，启发式）
- 阶段覆盖率（童年/求学/工作/家庭/转折/感悟）
...
| 变量名 | 说明 | 默认值 |
|-------|------|--------|
| MOONSHOT_API_KEY | Moonshot API密钥 | 必填 |
| MOONSHOT_BASE_URL | Moonshot网关地址 | https://api.moonshot.cn/v1 |
| MODEL_NAME | 使用的模型名称 | moonshot-v1-8k |
| LOG_LEVEL | 日志级别 | INFO |
