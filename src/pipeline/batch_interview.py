"""
Standalone batch interview pipeline with per-module timing.

Usage:
    python -m src.pipeline.batch_interview --config configs/batch_interview.yaml
    python -m src.pipeline.batch_interview \
...
def _timing_keys(mode: str) -> List[str]:
    base = [
        "interviewee_llm_ms", "interviewee_tool_ms",
        "interviewee_total_ms", "interviewer_llm_ms",
    ]
    planner = ["retrieval_ms", "extraction_ms", "write_ms"]
...
class InterviewRunner:
    ...
    def _build_agents(self):
        ...
        if self.cfg.interviewer_mode == "baseline":
            ...
            session_id = f"batch_{uuid.uuid4().hex[:10]}"
            interviewer = SessionOrchestrator(session_id=session_id, mode="graph_rag")
            interviewer.initialize_session(self.cfg.basic_info)
        return interviewee, interviewer

    def run_one(self, run_index: int) -> Dict[str, Any]:
        if self.cfg.interviewer_mode == "planner":
            return asyncio.get_event_loop().run_until_complete(
                self._run_one_planner(run_index)
            )
        return self._run_one_baseline(run_index)

    def _run_one_baseline(self, run_index: int) -> Dict[str, Any]:
    ...
    async def _run_one_planner(self, run_index: int) -> Dict[str, Any]:
        interviewee, orchestrator = self._build_agents()
        state = orchestrator._require_state()
        turns: List[Dict[str, Any]] = []
        transcript: List[str] = []

        last_question = state.pending_question or "您能讲讲自己的故事吗？"
        transcript.append(f"访谈者: {last_question}")
        ...
        for turn_idx in range(self.cfg.max_turns):
            ...
            timing.retrieval_ms = debug_trace.get("retrieval_ms")
            timing.extraction_ms = debug_trace.get("extraction_ms")
            timing.write_ms = debug_trace.get("write_ms")
