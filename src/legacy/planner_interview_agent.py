"""Legacy (pre-GraphRAG) planner interview agent wrapper."""
...
from src.legacy.orchestration.session_orchestrator import SessionOrchestrator

logger = logging.getLogger(__name__)


class LegacyPlannerInterviewAgent:
    ...
    def initialize_conversation(self, elder_info: Dict[str, Any]):
        ...
        logger.info("LegacyPlannerInterviewAgent initialized (session=%s)", self.session_id)

    async def get_next_question(self, user_response: str | None = None) -> Dict[str, Any]:
        if not self._initialized:
            raise RuntimeError("Agent must be initialized before use.")
...
class LegacyPlannerInterviewAgentSync:
    """Synchronous adapter for Flask handlers — mirrors PlannerInterviewAgentSync interface."""

    def __init__(self, session_id: str | None = None, decision_weights: Optional[Any] = None):
        self.async_agent = LegacyPlannerInterviewAgent(session_id, decision_weights=decision_weights)
