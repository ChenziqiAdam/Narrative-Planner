"""Legacy state models from pre-GraphRAG pipeline (snapshot: e46527e)."""
...
class GraphSummary:
    ...
    pending_themes: List[ThemeSummary] = field(default_factory=list)
    mentioned_themes: List[ThemeSummary] = field(default_factory=list)
    exhausted_themes: List[ThemeSummary] = field(default_factory=list)
...
class TurnRecord:
    ...
    turn_evaluation: Optional[Any] = None
    debug_trace: Dict[str, Any] = field(default_factory=dict)
...
class SessionState:
    ...
    evaluation_trace: List[Any] = field(default_factory=list)
