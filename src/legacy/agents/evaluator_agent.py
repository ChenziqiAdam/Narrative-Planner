...
class EvaluatorAgent:
    ...
    def _event_gain(self, turn_record: TurnRecord) -> float:
        ...
        if not extraction:
            return 0.0
        candidates = getattr(extraction.graph_delta, "event_candidates", None) or []
        if not candidates:
            return 0.0
        average_completeness = sum(getattr(event, "completeness_score", 0.5) for event in candidates) / len(candidates)
    ...
    def _slot_targeting_score(self, turn_record: TurnRecord, targeted_slots: List[str]) -> float:
        if not targeted_slots:
            return 0.7
        extraction = turn_record.extraction_result
        candidates = getattr(extraction.graph_delta, "event_candidates", None) if extraction else None
        if not extraction or not candidates:
            return 0.0

        hit_count = 0
        for slot_name in targeted_slots:
            if any(self._slot_has_value(event, slot_name) for event in candidates):
                hit_count += 1
        return hit_count / max(len(targeted_slots), 1)

    def _slot_has_value(self, event, slot_name: str) -> bool:
        if slot_name == "people":
            return bool(getattr(event, "people_ids", None) or getattr(event, "people_names", None))
        return getattr(event, slot_name, None) not in (None, "", [])

    def _emotional_alignment_score(self, state: SessionState, tone: str) -> float:
        memory_capsule = getattr(state, "memory_capsule", None)
        emotional_state = memory_capsule.emotional_state if memory_capsule else None
