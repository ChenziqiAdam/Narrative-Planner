...
你应当自由地从对话中提取实际存在的内容，而非试图填充某个预设模板。有什么就提什么，没有就不提。一次对话可以产生零个、一个或多个实体，每个实体可以连接零条或多条关系。
...
| 值 | 含义 | 何时使用 |
|------|------|------|
| `"new"` | 全新实体，图里没有对应的 | 对话中提到了 `existing_graph_context` 中不存在的人物、事件、地点等 |
| `"update"` | 对已有实体的更新/补充，**必须**填写 `merge_target_id` | 对话补充了已有实体的新细节，如老人对已提取的事件添加了时间、地点或感受 |
| `"skip"` | 已有实体已包含此信息，无需重复提取 | 老人重复讲了已有实体中已记录的内容 |
...
当 `existing_graph_context` 中存在已有实体时，对每个提取出的实体你必须判断：

1. **`merge_action: "new"`** — 全新实体，图里没有对应的
2. **`merge_action: "update"`** — 对已有实体的补充，必须填写 `merge_target_id` 为已有实体的 `id`
   - 此时 `description` 应**融合新旧信息**，不要只写新增部分
   - `name` 保持与已有实体一致
3. **`merge_action: "skip"`** — 已有实体已完整包含此信息，跳过不提取

**判断依据**：
- 名称相同或高度相似 + 描述内容是同一事物的不同方面 → `update`
- 完全重复已有内容 → `skip`
- 对话中出现了图里没有的新事物 → `new`

**关系处理**：关系中的 `source_name` / `target_name` 如果对应已有实体，使用已有实体的 `name`。
...
{
  "current_turn": {
    "interviewer": "在北大荒的时候，生活条件怎么样？",
    "respondent": "苦啊，零下三四十度，住在地窨子里，就是挖个坑上面盖上草。吃的就更不用说了，苞米面窝头，有时候连盐都没有。不过那时候年轻，也不觉得什么。"
  },
  "context": [],
  "existing_graph_context": [
    {"id": "event_abc123", "entity_type": "Event", "name": "离家赴北大荒", "description": "1958年18岁时响应号召离开家乡前往北大荒，母亲在县城火车站送别"}
  ]
}
```
  ...
  "entities": [
    {
      "entity_type": "Event",
      "name": "离家赴北大荒",
      "description": "1958年18岁时响应号召离开家乡前往北大荒，母亲在县城火车站送别。在北大荒住地窨子（挖坑盖草），零下三四十度，吃苞米面窝头，有时没有盐",
      "merge_action": "update",
      "merge_target_id": "event_abc123",
      "properties": {
        "living_conditions": "地窨子，零下三四十度",
        "food": "苞米面窝头，有时缺盐"
      }
    }
  ],
  "relationships": [],
  "narrative_summary": "老人补充了北大荒的生活细节：住地窨子，极寒天气，食物匮乏。",
  "open_loops": [],
  "confidence": 0.9
}
```
